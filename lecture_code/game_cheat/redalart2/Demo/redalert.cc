#include <map>  // Use a map to track original values
#include <cstdio>
#include <memory>
#include <cstring>
#include <string>
...
struct Game {
    ...
    std::map<uintptr_t, uint32_t> remain; // Watched addresses with initial values
            ...
            for (string line; std::getline(iss, line); ) {
                ...
                if (std::regex_search(line, match, r)) {
                    ...
                    for (uintptr_t off = 0; off < size; off += 4) {
                        if (mem[off / 4] == val) {
                            // Found a match! Store both address and initial value
                            remain[start + off] = val;
                        }
                    }
                }
            }
